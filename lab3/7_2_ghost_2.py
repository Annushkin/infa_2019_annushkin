...
def ellips_rot(x1, y1, a,b,phi):
    ...
    for x in range(-a,a,1):
        y=b*(1-x*x/(a*a))**0.5
        x,y=x*cos(radians(phi))-y*sin(radians(phi)), x*sin(radians(phi))+y*cos(radians(phi))
        xe=x+x1
        ye=y+y1
        p.append((xe, ye))
...
def gost_houses (x, y):
    ...
    penColor('#0d0c0c')
    brushColor('#0d0c0c')
    polygon([(x-10, y),(x+10, y-15),(x+120, y-15),(x+140, y),(x-10, y)])
    penColor('#282828')
    brushColor('#282828')
...
def ghosts (x, y, s):
    ...
    r=18
    circle(x, y, r*s)
    polygon([(x-r*s, y+0.4*r*s), (x-1.3*r*s, y+0.8*r*s), (x-1.4*r*s, y+1.5*r*s), \
            (x-2.6*r*s, y+3.9*r*s), (x-2.2*r*s, y+4.2*r*s), (x-2*r*s, y+4.8*r*s),\
            (x-1.1*r*s, y+4.4*r*s), (x+0.1*r*s, y+5.3*r*s), (x+1.1*r*s, y+4.3*r*s), \
            (x+2.2*r*s, y+4.9*r*s), (x+2.2*r*s, y+3.9*r*s), (x+4.4*r*s, y+4.4*r*s), \
            (x+1.7*r*s, y+0.3*r*s), (x+r*s, y), (x-r*s, y+0.4*r*s)])
    penColor('#87cdde')
    brushColor('#87cdde')
    r=5
